...
import {
  ...
  Circle, Square, ChevronDown, History, Dices, CircleDot,
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-center gap-3 flex-wrap">
        ...
        <Input type="number" min={1} max={50000} value={betAmount}
          onChange={(e) => { setBetAmount(Math.max(1, parseInt(e.target.value) || 1)); setError(""); }}
          disabled={spinning} className="w-24 h-8 text-xs text-center" data-ai="wheel-bet-custom" />
      </div>
        ...
        <div className="flex gap-1">
          {[10, 25, 50, 100].map(val => (
            <Button key={val} size="sm" variant={betAmount === val ? "default" : "outline"}
              ...
              className="text-xs" data-ai={`rps-bet-${val}`}>{val}</Button>
          ))}
        </div>
        <Input type="number" min={1} max={50000} value={betAmount}
          onChange={(e) => { setBetAmount(Math.max(1, parseInt(e.target.value) || 1)); setError(""); }}
          disabled={animating} className="w-24 h-8 text-xs text-center" data-ai="rps-bet-custom" />
      </div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-center gap-3 flex-wrap">
        ...
        <div className="flex gap-1">
          {[10, 25, 50, 100].map(val => (
            <Button key={val} size="sm" variant={betAmount === val ? "default" : "outline"}
              onClick={() => { setBetAmount(val); setError(""); }} disabled={flipping} className="text-xs">{val}</Button>
          ))}
        </div>
        <Input type="number" min={1} max={50000} value={betAmount}
          onChange={(e) => { setBetAmount(Math.max(1, parseInt(e.target.value) || 1)); setError(""); }}
          disabled={flipping} className="w-24 h-8 text-xs text-center" />
      </div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-center gap-3 flex-wrap">
        ...
        <div className="flex gap-1">
          {[10, 25, 50, 100].map(val => (
            <Button key={val} size="sm" variant={betAmount === val ? "default" : "outline"}
              onClick={() => { setBetAmount(val); setError(""); }} disabled={rolling} className="text-xs">{val}</Button>
          ))}
        </div>
        <Input type="number" min={1} max={50000} value={betAmount}
          onChange={(e) => { setBetAmount(Math.max(1, parseInt(e.target.value) || 1)); setError(""); }}
          disabled={rolling} className="w-24 h-8 text-xs text-center" />
      </div>
