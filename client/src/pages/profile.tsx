      ...
      <Card ref={profileCardRef} className="glass glass-border overflow-hidden mb-6 relative" style={cd.cardColor ? { borderColor: cd.cardColor + '40' } : undefined}>
        ...
        <CardContent className="relative z-10 pt-24 pb-6">
          <div className="flex flex-col md:flex-row items-start md:items-end gap-4">
            ...
            <div className="text-right space-y-2 group/actions">
              ...
              <div className="flex gap-1.5 justify-end flex-wrap">
                <button onClick={handleShareLink} disabled={sharing} className="group/btn inline-flex items-center gap-0 h-7 px-2 rounded-md border border-border/60 bg-background/40 hover:bg-white/10 transition-all duration-200 text-muted-foreground hover:text-white disabled:opacity-50" title={isRu ? 'Поделиться' : 'Share'}>
                  ...
                  <span className="text-xs font-medium max-w-0 overflow-hidden whitespace-nowrap group-hover/btn:max-w-[80px] group-hover/btn:ml-1 transition-all duration-200">{isRu ? 'Поделиться' : 'Share'}</span>
                </button>
                <button onClick={handleScreenshot} disabled={screenshotting} className="group/btn inline-flex items-center gap-0 h-7 px-2 rounded-md border border-border/60 bg-background/40 hover:bg-white/10 transition-all duration-200 text-muted-foreground hover:text-white disabled:opacity-50" title={isRu ? 'Скриншот' : 'Screenshot'}>
                  {screenshotting ? <Loader2 className="h-3.5 w-3.5 shrink-0 animate-spin" /> : <ImageIcon className="h-3.5 w-3.5 shrink-0" />}
                  <span className="text-xs font-medium max-w-0 overflow-hidden whitespace-nowrap group-hover/btn:max-w-[60px] group-hover/btn:ml-1 transition-all duration-200">{isRu ? 'Скрин' : 'Snap'}</span>
                </button>
                <button onClick={handleCopyId} className="group/btn inline-flex items-center gap-0 h-7 px-2 rounded-md border border-border/60 bg-background/40 hover:bg-white/10 transition-all duration-200 text-muted-foreground hover:text-white" title="Copy ID">
                  {copiedId ? <Check className="h-3.5 w-3.5 shrink-0" /> : <Copy className="h-3.5 w-3.5 shrink-0" />}
                  <span className="text-xs font-medium max-w-0 overflow-hidden whitespace-nowrap group-hover/btn:max-w-[30px] group-hover/btn:ml-1 transition-all duration-200">ID</span>
                </button>
                <button onClick={() => setShowDecorations(true)} className="group/btn inline-flex items-center gap-0 h-7 px-2 rounded-md border border-border/60 bg-background/40 hover:bg-white/10 transition-all duration-200 text-muted-foreground hover:text-white" title={isRu ? 'Декорации' : 'Decorations'}>
                  <Sparkles className="h-3.5 w-3.5 shrink-0" />
                  <span className="text-xs font-medium max-w-0 overflow-hidden whitespace-nowrap group-hover/btn:max-w-[60px] group-hover/btn:ml-1 transition-all duration-200">{isRu ? 'Декор' : 'Decor'}</span>
                </button>
                {!isOwnProfile && user?.discordId && (
                  <Link href={`/trading?target=${encodeURIComponent(profile.username)}`}>
                    <button className="group/btn inline-flex items-center gap-0 h-7 px-2 rounded-md border border-border/60 bg-background/40 hover:bg-white/10 transition-all duration-200 text-muted-foreground hover:text-white" title={isRu ? 'Обмен' : 'Trade'}>
                      <ArrowLeftRight className="h-3.5 w-3.5 shrink-0" />
                      <span className="text-xs font-medium max-w-0 overflow-hidden whitespace-nowrap group-hover/btn:max-w-[50px] group-hover/btn:ml-1 transition-all duration-200">{isRu ? 'Трейд' : 'Trade'}</span>
                    </button>
                  </Link>
                )}
              </div>
